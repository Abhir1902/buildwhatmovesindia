...
export function SourcePanel({ sources, jurisdiction }: { sources: ComplianceSource[]; jurisdiction: string }) {
  ...
  return (
    <aside className="mt-6 rounded-xl border border-neutral-200 bg-[var(--surface)] p-4 shadow-[var(--elev-2)]">
      <p className="font-mono text-[11px] tracking-wider text-neutral-500 uppercase">{t.common.source}</p>
      <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2">
        <div>
          <dt className="text-xs text-neutral-500">{t.common.source}</dt>
          <dd className="mt-1">
        ...
        <div>
          <dt className="text-xs text-neutral-500">{t.common.jurisdiction}</dt>
          <dd className="mt-1">{jurisdiction}</dd>
        </div>
        <div>
          <dt className="text-xs text-neutral-500">{t.common.lastVerified}</dt>
